...
export default function TrouverIntervenantPage() {
  ...
  const filteredIntervenants = intervenants.filter((intervenant) => {
    const matchesSearch =
      intervenant.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      intervenant.themes.some((theme) =>
        theme.toLowerCase().includes(searchQuery.toLowerCase())
      );
    const matchesCity =
      selectedCity === "all" || intervenant.cities.includes(selectedCity);
    const matchesTheme =
      selectedTheme === "all" || intervenant.themes.includes(selectedTheme);
    const matchesFormat =
      selectedFormat === "all" || intervenant.format === selectedFormat;
